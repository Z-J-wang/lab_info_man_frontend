  ...
  <div class="container">
    <div class="select-workflow-con">
      <Card style="text-align: left;">
        ...
        <Select v-model="workflow" size="large" label-in-value style="width: 400px;" @on-change="handleWorkflow">
        ...
        if (valid) {
          ...
          for (let i = 0; i < Object.keys(this.init_state.field_list).length; i++) {
            if ([25, 30].includes(this.init_state.field_list[i].field_type_id)) {
              data[this.init_state.field_list[i].field_key] = data[this.init_state.field_list[i].field_key].format(
                'yyyy-MM-dd hh:mm:ss'
              );
            }
